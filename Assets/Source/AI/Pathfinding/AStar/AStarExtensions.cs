...
namespace dEvine_and_conquer.AI.Pathfinding.AStar
{
    internal static class AStarExtensions
    {
        ...
        public static T Get<T>(this List<T> tiles, Point loc) where T : VisualObject => tiles.Where(x => (int)x.GetLocation().X == (int)loc.X && (int)x.GetLocation().Y == (int)loc.Y).First();
